...
public class VideoServer {
	 ...
	 */
	private void acceptConnection(final AsynchronousSocketChannel clientSocket) {
		HEADER.rewind();
		try {
	 ...
	 */
	private static final ByteBuffer HEADER = ByteBuffer
	 ...
	 */
	private final ArrayList<AsynchronousSocketChannel> clientSockets = new ArrayList<AsynchronousSocketChannel>(2);
	 ...
	 * @param quality
	 *            the quality of the JPEG stream
	 * 
	 * @throws IOException
	 ...
	 * 
	 * @throws IOException
	 *             if the server socket cannot be allocated
	 */
	 ...
	 *            the image to send
	 * @throws IOException
	 *             if the image could not be sent
	 */
					...
					for (int i = 0; i < clientSockets.size(); i++) {
						...
						if (future == null || future.isDone()) {
							try {
								...
								clientSocketFutures.set(i,
										cs.write(ByteBuffer.wrap(responseBufferOutputStream.toByteArray())));
